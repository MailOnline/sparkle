...
dropRatingValues rtgs =
  Spark.fromRDD =<<
  Spark.map (closure $ static (\(Tuple2 (Tuple2 u p) r) -> Tuple2 u p)) =<<
  Spark.toRDD =<<
  unpackRatings rtgs
...
rmse rdd1 rdd2 = do
  ...
  where
    mapSnd pairRdd =
      Spark.map (closure $ static (\(Tuple2 k v) -> v)) =<< toRDD pairRdd
...
main = do
  ...
  let options =
        [ ALSOptions factors 10 lambda
        | factors    <- [2, 5, 10, 20]
        , lambda     <- [0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0]
        ]

  forM_ options $ \alsOptions -> do
    als  <- newALS alsOptions
    mfm  <- runALS als training
    predicted <- predictMany validationInputs mfm
    r2 <- unpackRatings predicted
    (err, errCnt) <- rmse r1 r2
    print (alsOptions, err)
